...
class PariMCartes:
    ...
    #21,20,19: quasi sûr
    #18,17,16: risque
    #15,14,13,12,11,10,9: selon but
    #8,7,6,5,4,3,2: quasi nul
    #atout: 0/1
            ...
            if self.indice==(self.debut+1)%len(self.points):
                ...
                for carte in range(len(self.carte)):
                    ...
                    if self.carte[carte] == 'atout':
                        ...
                        for joueur in range(len(self.paris)):
                            if joueur != (self.indice - 1) % len(self.points) and joueur!=(self.indice-2)%len(self.points):
                                Prob = Proba(S, P[joueur], joueur, self.indice)
                                L[carte].append(Prob.pointsmax() * self.restegroupe[joueur])
                    elif self.carte[carte] < m:
                        g=maxi(TT)
        ...
        if len(self.carte)==1:
            ...
            for c in range(len(self.carte)):
                if self.carte[c]=='atout':
                    ...
                    ptg = [1-resteglob[k] * Coef[(self.debut - k-1) % len(self.resteglobal)] for k in range(len(self.resteglobal))]
                    ptg[self.indice]*=max(5-(22-self.carte[c])*0.7,0)
                    ptg=[k**2 for k in ptg]
                    ptg=[k/sum(ptg) for k in ptg]
                    Prb=[(Proba(L,Possibilités[k],k,self.indice)).pointsmax()*ptg[k] for k in range(len(self.resteglobal))]
                    Chances[c]=sum(Prb)
            return(sum(Chances))
