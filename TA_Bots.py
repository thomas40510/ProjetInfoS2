...
def pari1Carte(parisprécédents, cartesautresjoueurs):
    ...
    for k in cartesautresjoueurs:
        if k[0] == 'atout' \
                or k[0] > 13 + chgt * 2:  # parier 0 dans le cas: voir une carte trop forte chez les autres joueurs
            choix = 0
...
def PariMCartes(parisprécédents, cartes):
    ...
    for k in a:
        if k in parisautorisés:
            return k  # on renvoie l'entier le proche de ce que l'on veut parier, et que les règles autorisent
...
def ChoixMCartes(carte, paris, carteterrain, points, indice, debut):
    ...
    if risque == 'inf':
        ...
        if (TT == [0, 0] or TT == [0, 0, 0] or TT == [0, 0, 0,
                                                      0]) and reste != 0 and CC != []:
            # si l'on joue en premier, et que l'on a encore des plis à gagner, on pose la carte la plus forte
            return CC[-1]
        elif (TT == [0, 0] or TT == [0, 0, 0] or TT == [0, 0, 0,
                                                        0]) and reste == 0 and CC != []:
            # si l'on joue en premier, et que l'on ne doit plus gagner de plis, on pose la carte la plus faible
            return CC[0]
        elif CC == []:
            if reste > 0:
                return ['atout', 'maxi']
            ...
            # si possible, on se débarrasse des cartes fortes pour ne pas avoir à prendre de plis excédents avec
            if ['atout', 'maxi'] in TT:
                return CC[-1]
            else:
                M = max(TT)
...
def Proba(carte, resteglobal, debut, indice):
    """ Évalue de manière récursive si la situation donnée est favorable au bot (indice de favorabilité)
