...
# version ='1.0'
# ---------------------------------------------------------------------------
# Description : Module implémentant les décisions des bots
...
def pari1Carte(parisprécédents,cartesautresjoueurs):
    ...
    for k in cartesautresjoueurs:
        if k[0] == 'atout' or k[0] > 13 + chgt * 2: #parier 0 dans le cas: voir une carte trop forte chez les autres joueurs
            choix = 0
    if parisprécédents.count(-1)==1 and sum(parisprécédents)+choix==0: #si l'on est dernier à jouer, et que l'on a pas le choix
            choix=1-choix
    return(choix)
...
def PariMCartes(parisprécédents,cartes):
    ...
    if nbjoueursdéjàparié==nbjoueurs-1 and nbcartes-sumdéjàparié in parisautorisés:
        parisautorisés.remove(nbcartes-sumdéjàparié) #si l'on est le dernier à jouer, on retire le pari interdit des paris possibles
...
def ChoixMCartes(carte,paris,carteterrain,points,indice,debut):
    ...
    #risque évalue si l'on joue au supérieur (les paris ont été plus hauts que le nombre de cartes, donc les joueurs vont
    #se battre pour récupérer assez de plis), ou à l'inférieur (les paris ont été plus bas que le nombre de cartes, donc les
    #joueurs vont se battre pour ne pas récupérer trop de plis)
    #Les stratégies dans ces deux cas sont très différentes
    ...
    if risque=='inf':
        ...
        if (TT==[0,0] or TT==[0,0,0] or TT==[0,0,0,0]) and reste!=0 and CC!=[]: #si l'on joue en premier, et que l'on a encore des plis à gagner, on pose la carte la plus forte
            return(CC[-1])
        elif (TT==[0,0] or TT==[0,0,0] or TT==[0,0,0,0]) and reste==0 and CC!=[]: # si l'on joue en premier, et que l'on ne doit plus gagner de plis, on pose la carte la plus faible
            return(CC[0])
        elif CC==[]:
            if reste>0:
                return(['atout','maxi'])
                ...
                return(['atout','mini'])
        elif (reste==len(CC) and A==0) or (reste==len(CC)+1 and A==1): #si on doit gagner des plis, on pose la carte la plus forte
            return(CC[-1])
        ...
        if indice==debut: #si le bot joue en premier
            ...
            for c in range(len(carte)): #pour chaque carte
                ...
                for joueur in range(len(paris)):
                    ...
                    L[c].append(Prob*restegroupe[joueur]) #les estimations des situations selon la victoire de chaque joueur
...
def Proba(carte,resteglobal,debut,indice):
    ...
    if carte==[]: #initialisation
        ...
        Possibilités=possibilitésretirepoints(resteglobal) #situation au tour suivant selon la victoire de chaque joueur
        ...
        for c in range(len(carte)):
            if carte[c]=='atout': #certitude de gagner (peu d'intérêt de jouer l'atout en mini)
                ...
                ptg = [1-resteglob[k] * Coef[(debut - k-1) % len(resteglobal)] for k in range(len(resteglobal))] #chance de gagner le pli de chaque joueur
                ptg[indice]*=max(5-(22-carte[c])*0.7,0) #modification de cette chance pour le bot selon la carte qu'il pose
                ptg=[k**2 for k in ptg]
                ptg=[k/sum(ptg) for k in ptg] #normalisation
                Prb=[(Proba(L,Possibilités[k],k,indice))*ptg[k] for k in range(len(resteglobal))] #hérédité: produit de l'indice de favorabilité de chaque situation par la probabilité qu'elle arrive
